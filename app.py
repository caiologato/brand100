...
with tab_plan:
    ...
    if not selected_stores.empty:
        ...
        with row2_col2:
            st.markdown("**Concentração de Impacto por Cidade/UF**")
            df_geo = selected_stores.groupby(['Cidade', 'UF'])['Alcance Total'].sum().reset_index().sort_values('Alcance Total', ascending=False).head(10)
            fig_geo = px.bar(df_geo, x='Alcance Total', y='Cidade', orientation='h', text_auto='.2s', color='UF', color_discrete_sequence=px.colors.qualitative.Prism)
            fig_geo.update_layout(title_text='Top 10 Cidades com Maior Alcance', margin=dict(t=40, b=0, l=0, r=0), height=350, yaxis={'categoryorder':'total ascending'})
...
with tab_admin:
    ...
    if not st.session_state['autenticado']:
        ...
        if st.button("🔓 Entrar na Área Restrita", use_container_width=True):
        ...
        with admin_tab3:
            ...
            with col_acao:
                ...
                lojas_selecionadas = edited_precos[edited_precos['Selecionar']]['Loja'].tolist()
