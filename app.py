...
# 2. Inicialização do Banco de Dados (Apenas para Propostas e Comerciais)
def init_db():
...
# 3. Carregamento dos Dados (Lendo direto do Excel e Preço Global da Sessão)
@st.cache_data
def load_excel_data():
...
try:
    if 'df_base' not in st.session_state:
        st.session_state['df_base'] = load_excel_data()
...
with tab_plan:
    ...
    if not selected_stores.empty:
        ...
        with row2_col2:
            ...
            if not df_map.empty:
                # MAPA ESTÁTICO DE ALTA PERFORMANCE: Renderiza centenas de pontos sem falhar
                fig_map = px.scatter(
                    df_map, x="Lon", y="Lat", hover_name="Loja", size="Alcance Total",
    ...
    if user == "cencomedia" and senha == "brand100":
        ...
        with admin_tab1:
            ...
            if not df_propostas.empty:
                ...
                busca_hist = col_busca.text_input("🔍 Buscar no histórico...", "")
                ...
                edited_propostas = st.data_editor(
                    ...
                    column_config={
                        ...
                        "status": st.column_config.SelectboxColumn("Status", options=["Enviada", "Sem Retorno", "Em revisão", "Aprovada", "Reprovada", "Cancelada"], required=True),
                ...
                if st.button("💾 Salvar Alterações no Funil"):
                    ...
                    conn.commit()
                    conn.close()
                    st.success("Tabela de CRM atualizada!")
                ...
                st.info("Nenhuma proposta salva no funil ainda.")
        ...
        with admin_tab3:
            ...
            if col_b1.button("✅ Selecionar Todas as Lojas", use_container_width=True):
                st.session_state['df_base']['Selecionar_Admin'] = True
            if col_b2.button("🟩 Desmarcar Todas as Lojas", use_container_width=True):
                st.session_state['df_base']['Selecionar_Admin'] = False
                
            if 'Selecionar_Admin' not in st.session_state['df_base'].columns:
                st.session_state['df_base']['Selecionar_Admin'] = False

            df_admin_precos = st.session_state['df_base'][['Selecionar_Admin', 'Loja', 'Valor Diária Base (R$)']].copy()
            ...
            with col_tabela:
                edited_precos = st.data_editor(
                    df_admin_precos,
                    column_config={
                        "Selecionar_Admin": st.column_config.CheckboxColumn("Selecionar", default=False),
            ...
            with col_acao:
                ...
                # Atualiza a seleção no estado global
                st.session_state['df_base']['Selecionar_Admin'] = edited_precos['Selecionar_Admin']
                
                lojas_selecionadas = edited_precos[edited_precos['Selecionar_Admin']]['Loja'].tolist()
                st.info(f"{len(lojas_selecionadas)} loja(s) selecionada(s).")
                
                novo_preco_lote = st.number_input("Novo Preço para Selecionadas (R$)", min_value=0.0, value=349.0, step=10.0)
                
                if st.button("💾 Aplicar Preço nas Selecionadas", use_container_width=True):
                    if len(lojas_selecionadas) > 0:
                        for loja in lojas_selecionadas:
                            mask = st.session_state['df_base']['Loja'] == loja
                            st.session_state['df_base'].loc[mask, 'Valor Diária Base (R$)'] = novo_preco_lote
