...
# Nome do banco atualizado (v3) para comportar as novas colunas de CRM (Status e Follow-up)
DB_NAME = 'cencosud_dooh_v3.db'
...
def init_db():
    ...
    c = conn.cursor()
    # Tabela de propostas atualizada com Status e Data Follow-up
    c.execute('''CREATE TABLE IF NOT EXISTS propostas
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, data_criacao TEXT, comercial TEXT, nome_plano TEXT, 
                  cliente TEXT, inicio TEXT, fim TEXT, investimento REAL, impactos REAL, status TEXT, data_followup TEXT)''')
...
def load_data():
    ...
    df_ui = pd.DataFrame({
        'Selecionar': False,
        'Status': df['Status'].fillna('Não Informado').astype(str),
...
with tab_plan:
    ...
    status_disponiveis = df_ui['Status'].unique().tolist()
    ...
    if not selected_stores.empty:
        ...
        with row2_col2:
            df_map = selected_stores.copy()
            df_map['Lat'] = pd.to_numeric(df_map['Lat'], errors='coerce')
            df_map['Lon'] = pd.to_numeric(df_map['Lon'], errors='coerce')
            df_map = df_map.dropna(subset=['Lat', 'Lon'])
            df_map = df_map[df_map['Alcance Total'] > 0]
        ...
        with save_col2:
            st.write("") # Espaço para alinhar com o campo de data
            st.write("")
            if st.button("Salvar no Sistema de Vendas", use_container_width=True):
                if not nome_plano or not nome_cliente:
                    ...
                    conn = sqlite3.connect(DB_NAME)
                    c = conn.cursor()
                    c.execute("INSERT INTO propostas (data_criacao, comercial, nome_plano, cliente, inicio, fim, investimento, impactos, status, data_followup) VALUES (?,?,?,?,?,?,?,?,?,?)",
                              (datetime.today().strftime("%Y-%m-%d %H:%M"), dados_comercial['nome'], nome_plano, nome_cliente, str(data_inicio), str(data_fim), total_liquido, total_impactos, "Enviada", str(data_followup_input)))
                    conn.commit()
                    conn.close()
                    st.success("✅ Proposta enviada para o funil!")

        # Ação 2: Exportar Excel
        output = io.BytesIO()
            ...
            if not df_propostas.empty:
                ...
                edited_propostas = st.data_editor(
                    ...
                    column_config={
                        ...
                        "comercial": st.column_config.Column("Comercial", disabled=True),
                ...
                # BOTAO DE DOWNLOAD DO FUNIL RESTAURADO E EM EXCEL
                st.write("---")
                output_funil = io.BytesIO()
                with pd.ExcelWriter(output_funil, engine='xlsxwriter') as writer:
                    # Copiando para evitar alertas de edição
